			...
			<el-table
				...
				<el-table-column label="操作" width="300" :resizable='true' :sortable='false' align="left" header-align="left">
					<template #default="scope">
						...
						<el-button v-if="btnAuth('dingdanxinxi','配送')" type="success" @click="dingdanpeisongCrossAddOrUpdateHandler(scope.row,'cross','','','')">配送</el-button>
					</template>
				</el-table-column>
			</el-table>
			<el-pagination 
...
<script setup>
    ...
    const dingdanpeisongCrossAddOrUpdateHandler = (row,type,crossOptAudit,statusColumnName,tips,statusColumnValue) => {
		...
		nextTick(()=>{
			dingdanpeisongFormModelRef.value.init(row.id,'cross','配送',row,'dingdanxinxi',statusColumnName,tips,statusColumnValue)
		})
    }
	//初始化
	const init = () => {
